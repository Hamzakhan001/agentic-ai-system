...
async def review_documents(body: ReviewRequest, session: Session = Depends(get_session)) -> ReviewResponse:
    with tracer.start_as_current_span("review.run") as span:
        ...
        try:
            ...
            span.set_attribute("review.final_task_type", result["task_type"])
            span.set_attribute("review.extracted_facts_count", len(result.get("extracted_facts", [])))
            span.set_attribute("review.sources_count", len(result.get("sources", [])))
